...
export class MasksManager {
    ...
    private getMaskPreview(): void {
        // figure out what is in layer 1? Is it regions or mask
        const isRegionsFirst = true;

        // figure max layer count.
    ...
    private getAndLoadMasks(layer: Konva.Layer, layerNumber: number): void {
        ...
        this.loadMasksInternal(allMasks, layer, layerNumber);
    }

    private loadMasksInternal(allMasks: IMask[], layer: Konva.Layer, layerNumber?: number) {
        const currentDimensions = this.getCurrentDimension();
